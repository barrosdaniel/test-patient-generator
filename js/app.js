...
function generateMedicareNumbers() {
  ...
  if (quantity < 1 || quantity > 100) {
    window.alert(
      "JS ERROR: The number has to be between 1 and 100. Try again."
    );
...
function generatePassword() {
  ...
  if (validateCheckboxes() === false) {
    window.alert(
      "JS ERROR: You need to select at least one character type to include in your password. Try again."
    );
  ...
  if (characters < 9 || characters > 64) {
    window.alert(
      "JS ERROR: The number of characters has to be between 9 and 64. Try again."
    );
    numCharacters.value = "";
    passAppDataContainer.style.display = "none";
    return;
...
// Selectors
const testPatAppQuantity = document.querySelector(
  ".test-pat-app__qty-selection-input"
);
const testAppChildrenOnlyCheck = document.getElementById("childrenOnly");
...
const testPatAppDataTitle = document.querySelector(
  ".test-pat-app__data-string-title"
);
...
function createDOBData() {
  const regularPatientDOB = new Date("1915-01-01");
  const childrenPatientDOB = new Date("2005-01-01");
  const DOBDataSpan = document.createElement("span");
  let node = document.createTextNode("");
  if (testAppChildrenOnlyCheck.checked) {
    node = document.createTextNode(getRandomDate(childrenPatientDOB));
  } else {
    node = document.createTextNode(getRandomDate(regularPatientDOB));
  }
...
function getRandomDate(pastDate) {
  const today = new Date();
  const randomTime =
    (today.getTime() - pastDate.getTime()) * Math.random() + pastDate.getTime();
  const randomDate = new Date(Math.ceil(randomTime)).toLocaleDateString();
  return randomDate;
}
